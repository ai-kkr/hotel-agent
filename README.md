...
обойтись нельзя. Агент также умеет **планировать** свои ходы на будущее (отложенно или по расписанию
через Temporal Schedules) — например, напомнить отелю о раннем заезде накануне или раз в неделю
проверять ответ.
...
> **Дев и прод разведены.** Прод (Railway) и дев (локальный/NAS) используют **разные Telegram
> bot-token'ы** и разные окружения — это изолированные контуры. Поэтому редеплой/запуск прода не
> требует остановки чего-либо, и наоборот. (Раньше тут было неверное правило «остановить другой
> инстанс» — оно устарело.)
