...
- **Migrations with the JSON column type.** When a custom `TypeDecorator` (e.g.
  `src.db.types.MessageDetailsType`) is referenced in an auto-generated migration, alembic does
  not add the import — add `import src.db.types` to the migration file by hand.
