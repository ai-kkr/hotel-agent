# Spec: chat-model-providers

## Purpose

Конструирование единой чат-модели (`BaseChatModel`) из конфигурации для всех
агентов конвейера KKR-Hotel. Фабрика `build_model` поддерживает несколько
LLM-провайдеров (как минимум **openai** и **zai** — Z.AI, OpenAI-compatible),
выбираемых по значению `Settings.llm_model` (префикс `<provider>:<model>` или
дефолт), с валидацией учётных данных и параметризуемым endpoint.
