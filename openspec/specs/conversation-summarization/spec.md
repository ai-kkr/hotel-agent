...
When a guest↔agent thread grows toward the model's context window, the agent auto-compresses the
old message prefix into a running summary stored in a dedicated state field, so long hotel/email
conversations never hit the context limit and prompt-cache locality is preserved. The trigger is
reactive — read from the model's own reported `usage_metadata["input_tokens"]` after each call, not
from a local tokenizer — and the summarization LLM call is shaped as a cache continuation of the
long thread (identical model config, immutable prefix). The summary leads with the agent's current
goal and immediate next step so it never loses the thread of what it was doing.
...
fetch the chat model lazily (built identically to the `model` node — same provider config, same
bound tools, same OpenRouter sticky session — so the summarization call is a cache continuation of
the long thread, not a separate request) and SHALL NOT pass any live object through `EmailState`
or `EmailContext`.

#### Scenario: summarize activity runs under its own timeout and builds the model identically to model_node

- **WHEN** the `summarize` node runs
- **THEN** it executes as a Temporal activity with a `start_to_close_timeout` covering an LLM call,
  builds the chat model with the same config + bound tools + sticky session as the `model` node,
  and stores no live object in state or context
