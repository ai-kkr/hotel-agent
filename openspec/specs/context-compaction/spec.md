...
The agent graph compacts disposable tool output at the end of each turn so that heavy
search/scrape payloads do not accumulate in persisted `EmailState.messages`. A `cleanup`
step replaces whitelisted `ToolMessage` content in place with a short archived stub,
preserving message identity and threading. Compaction is idempotent, selected strictly by
tool name, and operates only on state — no live objects cross the workflow↔activity
boundary.
