...
Дев и прод — изолированные контуры с разными Telegram bot-token'ами, так что останавливать
другой контур при деплое/запуске не нужно. Ограничение «один token — один процесс» действует
только внутри одного контура (два процесса с одним token'ом получат `409 Conflict`).
...
- `tool.*` — вызовы инструментов (`tool.set_booking_info`, `tool.send_wishes_to_hotel`,
  `tool.reply_to_hotel`, `tool.search_internet`, `tool.set_scheduled_task`, …) с релевантными
  полями;
- `agent.self_correction` / `agent.tool_validation` — тула отказала по предусловию
  (`SelfCorrectionError`) или невалидным аргументам (`pydantic.ValidationError`); обёртка
  превратила это в `ToolMessage`-подсказку, ход продолжается;
